...
async function aplicarCupom() {
  const cupomEl = el("cupom");
  if (!cupomEl) return;
  
  const codigo = cupomEl.value.trim().toUpperCase();
  if (!codigo) {
...
if (seletorPagamento) {
  seletorPagamento.addEventListener("change", (e) => {
    document.querySelectorAll(".payment-option").forEach(elem => elem.classList.remove("active"));
...
el("btn-logout")?.addEventListener("click", async () => {
  await signOut(auth);
  if (el("checkout-status")) el("checkout-status").textContent = "Desconectado com sucesso";
});

// ==================== API URL SELECTOR (DEV) ====================
...
const apiEnvSelect = el("api-env-select");
if (apiEnvSelect) {
  apiEnvSelect.value = localStorage.getItem("zuca_api_env") || "local";
  apiEnvSelect.addEventListener("change", (e) => {
    localStorage.setItem("zuca_api_env", e.target.value);
    alert(`API alterada para: ${e.target.value}. Recarregue a página.`);
    location.reload();
  });
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (el("auth-status")) el("auth-status").textContent = `Logado como ${user.email || user.displayName || "cliente"}.`;
