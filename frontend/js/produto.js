...
(async function modelosReceituario() {
  ...
  const ICONE_PATHS = ICONES_PATHS; // paths definidos em icones-paths.js
  ...
  // Suporta: string (viewBox 24×24) e objeto { paths, vw, vh } (viewBox customizado).
  function desenharIconeCanvas(ctx2d, tipo, cx, cy, tamanho, cor) {
    const def = ICONE_PATHS[tipo];
    if (!def) return;
    ...
    const s     = tamanho / vw;
    const drawH = tamanho * (vh / vw);
    ctx2d.save();
    ctx2d.translate(cx - tamanho / 2, cy - drawH / 2);
    ctx2d.scale(s, s);
    ctx2d.fillStyle = cor;
    for (const d of paths) ctx2d.fill(new Path2D(d));
    ctx2d.restore();
  }
